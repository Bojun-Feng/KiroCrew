 ...
 *   11-dismiss-refused     a 409 from a single row's dismiss endpoint
...
const apiFor = ({
  ...
  if (path.endsWith('/dismiss-all')) {
    await json(
      route,
      dismissAllStatus === 200
        ? { ok: true, dismissed: PENDING.length }
        : { error: 'candidate directory is read-only', code: 'not_writable' },
      dismissAllStatus,
    )
    return true
  }
...
const LEAD = {
  ...
  'dismiss-all': () => "Couldn't dismiss all pending skills.",
}
...
try {
  ...
  {
    const page = await open()
    await stubDashboardApi(page, { extra: apiFor({ dismissStatus: 409 }) })
    await page.goto(`${base}/capabilities?tab=skills`, { waitUntil: 'networkidle' })
    await page.getByText('rotate-fixtures-update').first().waitFor()
  ...
  {
    const page = await open()
    await stubDashboardApi(page, { extra: apiFor({ dismissAllStatus: 409 }) })
    await page.goto(`${base}/capabilities?tab=skills`, { waitUntil: 'networkidle' })
    await page.getByText('rotate-fixtures-update').first().waitFor()
