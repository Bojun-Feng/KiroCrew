...
describe('useWebSocket goal-loop seed vs live frames', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(api.autonudgeList).mockReset().mockImplementation(() => seedDeferred.promise as ReturnType<typeof api.autonudgeList>)
  ...
  it.each(['slack:123.456', 'dashboard:dashboard_dashboard_slack:123.456'])('folds seed and live frame keys for sidebar rows: %s', async slot => {
    renderHook(() => useWebSocket(), { wrapper })
    act(() => { WS_INSTANCES[0].simulateOpen() })

    await act(async () => {
      seedDeferred.resolve({ enabled: true, loops: [{ ...LOOP, slot_key: slot }] })
      await seedDeferred.promise
    })
    expect(loops()).toEqual({ 'slack_123.456': { cycle_count: 7, max_cycles: 24 } })

    act(() => {
      WS_INSTANCES[0].simulateMessage({
        type: 'autonudge_state',
        data: { event: 'fired', slot, loop: { ...LOOP, cycle_count: 9 } },
      })
    })
    expect(loops()).toEqual({ 'slack_123.456': { cycle_count: 9, max_cycles: 24 } })
  })

  it('projects live structured monitor budgets into sidebar progress', () => {
    renderHook(() => useWebSocket(), { wrapper })
    act(() => { WS_INSTANCES[0].simulateOpen() })
    ...
    act(() => {
      WS_INSTANCES[0].simulateMessage({
        type: 'autonudge_state',
        data: {
          event: 'fired', slot: 'chat-1-1721', loop: {
            ...LOOP, cycle_count: 99, max_cycles: 99,
    ...
    expect(loops()['chat-1-1721']).toEqual({ cycle_count: 3, max_cycles: 8 })
  })

  it('discards an older seed when overlapping reconnect seeds resolve in reverse order', async () => {
    vi.useFakeTimers()
    // Reconnect also fetches slots; keep this row live during reconciliation.
    vi.mocked(api.chatSlots).mockResolvedValueOnce([
      { key: LOOP.slot_key, title: 'Loop', messages: 0, running: false, mode: '', created: '', last_ts: '' },
    ])
    const first = seedDeferred
    ...
    await act(async () => {
      seedDeferred.resolve({ enabled: true, loops: [{ ...LOOP, cycle_count: 9 }] })
      await seedDeferred.promise
    })
    expect(loops()).toEqual({ 'chat-1-1721': { cycle_count: 9, max_cycles: 24 } })

    await act(async () => {
      first.resolve({ enabled: true, loops: [LOOP] })
      await first.promise
    })
    expect(loops()).toEqual({ 'chat-1-1721': { cycle_count: 9, max_cycles: 24 } })
  })
