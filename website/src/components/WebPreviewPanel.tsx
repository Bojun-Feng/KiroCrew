 ...
 *  - `'policy'`: the engine would allow it, but the dashboard's own CSP
 *    `frame-src` carries no entry for the host (see `EMBEDDABLE_LOOPBACK_HOSTS`),
 *    so the frame would render blank. That is a gateway policy, not a browser one.
 *
    ...
    <div className="relative flex flex-col h-full min-h-0 bg-bg">
      ...
      <div className="relative flex-1 min-h-0 bg-white">
        {launch ? (
          ...
          // `'policy'` is the dashboard's own frame-src (a `*.localhost` or other
          // trustworthy host it carries no entry for). Both keep the
