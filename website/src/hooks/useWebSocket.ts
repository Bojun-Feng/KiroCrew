...
export function useWebSocket() {
  ...
  /** Bumped by every `autonudge_state` frame AND every seed. A seed captures
   *  its generation and discards the response if a frame or newer seed arrived:
  ...
  const seedGoalLoops = useCallback(() => {
    ...
    try {
      ...
      api.autonudgeList()
        .then(res => {
          // A live frame or newer seed superseded this snapshot — drop it
          ...
          dispatch(setGoalLoops((res?.loops || []).map(loop => ({
            slot: dashboardGoalLoopSlotKey(loop.slot_key),
          ...
          case 'autonudge_state': {
            ...
            const nudge = data as unknown as {
              ...
              loop?: {
                ...
                cycle_count?: number
                max_cycles?: number
                monitor?: { agent_turns?: number; budgets?: { max_agent_turns?: number } }
              }
            }
            if (nudge.slot) {
              ...
              dispatch(sseGoalLoop({
                slot: dashboardGoalLoopSlotKey(nudge.slot),
                ...
                cycle_count: Number(nudge.loop?.monitor?.agent_turns ?? nudge.loop?.cycle_count) || 0,
                max_cycles: Number(nudge.loop?.monitor?.budgets?.max_agent_turns ?? nudge.loop?.max_cycles) || 0,
              }))
            }
