...
import { dashboardGoalLoopSlotKey } from '../utils/dashboardSlotKey'
...
export default function ChatPage({ mode, embedded, embedMode, popout, noUrlSync }: { mode?: string; embedded?: boolean; embedMode?: 'chat' | 'sessions'; popout?: boolean; noUrlSync?: boolean } = {}) {
  ...
  const [autoNudgeLoadFailed, setAutoNudgeLoadFailed] = useState(false)
  const [autoNudgeLoadRetry, setAutoNudgeLoadRetry] = useState(0)
  const retryAutoNudgeLoopLoad = useCallback(() => setAutoNudgeLoadRetry(retry => retry + 1), [])
  ...
  useEffect(() => {
    ...
    setAutoNudgeLoadFailed(false)
  }, [activeSlot])

  // The snapshot fetch lives in its own effect so a Retry press re-runs only
  // the fetch (via the retry counter) without wiping the loop already shown.
  useEffect(() => {
    ...
    const loadLoop = async () => {
      try {
        const response = await fetch('/api/autonudge/slot/' + encodeURIComponent(activeSlot))
        if (!response.ok) throw new Error('HTTP ' + response.status)
        const data = await response.json()
        if (!cancelled && !superseded) {
          setAutoNudgeLoop(data.loop || null)
          setAutoNudgeLoadFailed(false)
        }
      } catch {
        if (!cancelled && !superseded) setAutoNudgeLoadFailed(true)
      }
    }
    void loadLoop()
    const onEvent = (e: Event) => {
      const detail = (e as CustomEvent).detail as { slot?: string; loop?: AutoNudgeLoop; event?: string }
      if (!detail?.slot || dashboardGoalLoopSlotKey(detail.slot) !== dashboardGoalLoopSlotKey(activeSlot)) return
      // A live frame is fresher than the snapshot requested before it.
      // Do not let that in-flight response overwrite the live state.
      superseded = true
        ...
        {splitMode && splitFeatureEnabled ? (
          ...
          <div className="relative flex flex-col flex-1 min-h-0" {...dropTargetProps}>
            ...
            <div className="relative">
              ...
              autoNudgeLoadFailed={autoNudgeLoadFailed}
              onRetryAutoNudgeLoopLoad={retryAutoNudgeLoopLoad}
