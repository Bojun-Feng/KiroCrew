...
was added 2026-08-27. The `rfc-crew-agent-sdk-boundary` row was added 2026-08-28 and verified against `dc88f142b`. The `rfc-transcript-section-markers` row was added 2026-08-30 and verified against `202770d13`. The `rfc-mcp-lifecycle-event-log` row was added 2026-09-01 and verified against `1ee69f225`. The `rfc-session-tag-change-event` row was added 2026-09-02 and verified against `6581a04ee`. The `rfc-agent-config-mirror` row was added 2026-09-02 and verified against `f8b3203c0`. The `rfc-chat-core-extraction` row was added 2026-09-05 and verified against `8ed028b0b`. The `rfc-conductor-work-ledger` row was added 2026-09-05 and verified against `049b8c729`. The `rfc-app-session-controls` row was added 2026-08-31 and re-verified against `1d705a03f` on 2026-09-01. The `rfc-central-governance-ceiling` row was added 2026-09-08 and verified against `8a9c269b4`. The `rfc-security-conductor` row was re-audited 2026-09-10 against `f6d38741c`. The `rfc-append-only-ledger` row was added 2026-09-11 and verified against `2f1e2f54a`, and moved to `in-progress` the same day against the open PR named in it rather than against main.

| Document | Status | What is actually on main |
|---|---|---|
| [rfc-org-chart-and-guardrails.md](rfc-org-chart-and-guardrails.md) | `in-progress` | Implementation is in open PR [#10595](https://github.com/kirodotdev/KiroCrew/pull/10595), not on main: persistent organization members, fixed role limits, bounded hiring, chat-originated assignments and explicit review. Source and callers verified at `e568a6621`. The owning runtime contract is `system-specs/modules/crew-mode.md`. |
| [rfc-append-only-ledger.md](rfc-append-only-ledger.md) | `in-progress` | Nothing on main; PR 1 is [#10091](https://github.com/kirodotdev/KiroCrew/pull/10091). That PR is `kiro_crew.ledger` (the per-unit files for both kinds under a masked `ledgers/` root, an envelope field-compatible with `kiro_crew.events` plus `seq`/`thread`/`ref`, the headers, type ownership, guest namespaces, paging, `ok`/`gone` resolution, torn-tail repair with deterministic closers) with the flag-gated session emitter as its first writer; that PR carries its own module specs. Verified at `2f1e2f54a`, before it: `members.py` appends `{ts, member, session}` entries carrying neither `type` nor `seq`, `work_ledger.py` (`items/<id>.json`) and `session_ledger.py` (`state.json`) each keep a separate head, and `src/kiro_crew/events/` has an envelope with no writer. PR 2 — projections, routes, UI, the crew writer and migration — is unstarted |
