...
| Crew member inbox model (per-member opt-in) | A flagged member (`members.<slug>.inbox_model: true`) stops running its DM thread as one long-lived turn: what the person types becomes a `user_dm` envelope in an on-disk inbox, a per-member scheduler wakes the member in an ephemeral `member-<slug>.wake-<n>` execution context whose MODEL context is the member's one durable provider conversation (`members.<slug>.wake_context: resume`, default; each wake appends one turn, so the model keeps a live thread's conversational memory; `fresh` = ephemeral; a conversation that cannot be resumed is restarted with a `wake_context_reset` system row in the projection), the reply is written as an outbox row and mirrored into the thread, and members message each other with `peer_send` (causal hop cap, per-member budget refilled only by the owner, per-pair rate limit). M1: the thread renders the PROJECTION of the member's inbox + outbox — the person's `user_dm` and the member's `reply` rows as bubbles, `session_dm` / `peer_dm` / `worker_report` / `wake_timer` / `system` rows as compact badged rows folded per kind and direction and collapsed until expanded (direction arrow per peer message, tooltips on the kind badge, hop and state); the projection is a read-only view — no Quote / Ask selection footer and no pinned rows, which stay on the DM transcript; viewing marks read (monotone marker keyed on the row rendered last, served window covers every unread row), and the roster row carries the durable unread count; a session a member created reports this turn's reply or failure back as a `worker_report` envelope, budgeted per member until the owner writes again. M2: a conductor-class member (one holding a work ledger under its key) gets a `[conductor work ledger]` snapshot in every wake prompt, and `monitor_start` on a wake is refused with a reason that names the scheduler as the member's cadence | Crew Members thread (composer unchanged — typing mints the `user_dm`); `session_send` to a flagged member's thread is translated to an envelope; roster row badge | `pages/members/MembersPage.tsx` (mounts the projection through `ChatPane`'s `transcript` override), `pages/members/MemberProjection.tsx` (the projection view), `pages/members/projection.ts` (pure grouping / marker helpers — no feature of its own) | `src/kiro_crew/member_inbox.py` (`projection`, read marker), `src/kiro_crew/member_scheduler.py`, `src/kiro_crew/member_peer.py`, `member_wake.py` (`report_worker_turn`, wake prompt), `src/kiro_crew/work_ledger.py` (`render_snapshot`), `src/kiro_crew/autonudge_authz.py` (`external_arm_refusal`), `handlers/member_inbox.py`, `handlers/members.py` (projection / read), `chat_handlers.py` (`_member_inbox_intake`), `chat_runner.py` (turn-end hook `_report_worker_turn_end`, before the queue drain), `session_control.py` (`_inbox_model_shim`) | `POST /api/chat` (flagged member slot → `{inbox: true}`), `GET /api/members/{slug}/projection`, `POST /api/members/{slug}/read`, `GET /api/members` (`inbox_model`, `unread`), `POST /api/member-inbox/outbox`, `POST /api/member-inbox/peer-send` (strict-internal, MCP tools `outbox_send` / `peer_send`) |
...
| `chat` | Chat behavior preferences, plain vs highlighted diffs | `ChatPanel.tsx` | `handlers/core.py`, `handlers/files.py` | `GET,PUT,PATCH /api/config/kirocrew`, `GET,PUT /api/dashboard/config` |
