...
| Session control | Create / stop / send-to a session from outside it. Two `send`-only allows in front of the `not_creator` ownership fence: a crew member may send to another member's DM thread, and a session may send to the session that created it (`create` / `stop` / `close` / `read` stay refused). Every delivered row carries a gateway-built `meta.sent_by` record (`session_key`, `title`, `agent`, `via`, `member_slug?`, `child?`) beside the unchanged `[sent by session …]` text prefix; a BUSY member target is steered into its running turn rather than queued (`deliver_sent_by`), and the result reports `steered` beside `started`. No hop cap and no pair rate limit on member↔member traffic (owner decision) | Agent and app callers, not a UI | — | `session_control.py` (`authorize_target`, `send_to_target`, `deliver_sent_by`, `sent_by_meta`), `chat_delivery.py` (`steer_into_running_turn` `sent_by`) | `POST /api/session-control/create`, `.../stop`, `.../send`, `GET .../read` |
| Agent chat egress | Post, edit and delete the bot's own chat messages from an agent turn. `send_message(session="origin")` from a NON-cron caller resolves to the session that created the caller (`_created_by`) and lands there as a `meta.sent_by` row (`via: send_message_origin`) through the session-control delivery; only a caller with no creator, or a refused or vanished creator, falls back to the bell; a cron caller's origin is still its job's session. For POST and EDIT, a room target must be in the tracked-channel allowlist and a DM must be the owner's resolved DM, and outbound text and blocks pass the display-form redaction floor before reaching the provider. DELETE carries NO target authorization today: it validates only that `channel` and `ts` are present and forwards to the provider, so it reaches any message the bot token can delete (documented gap, not a described boundary) | Agent-written via the `kirocrew-core` messaging MCP tools; no dashboard page | — | `handlers/messaging.py` | `POST /api/send-message`, `POST /api/update-message`, `POST /api/delete-message` |
