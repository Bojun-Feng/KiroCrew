...
| Crew member inbox model (per-member opt-in) | A flagged member (`members.<slug>.inbox_model: true`) stops running its DM thread as one long-lived turn: what the person types becomes a `user_dm` envelope in an on-disk inbox, a per-member scheduler wakes the member in an ephemeral `member-<slug>.wake-<n>` execution context whose MODEL context is the member's one durable provider conversation (`members.<slug>.wake_context: resume`, default; each wake appends one turn, so the model keeps a live thread's conversational memory; `fresh` = ephemeral; a conversation that cannot be resumed is restarted with a `wake_context_reset` system row in the projection), the reply is written as an outbox row and mirrored into the thread, and members message each other with `peer_send` (causal hop cap, per-member budget refilled only by the owner, per-pair rate limit). Backend only in this milestone — the thread keeps its existing rendering; a projection view with envelope-kind badges and an unread badge follows | Crew Members thread (composer unchanged); `session_send` to a flagged member's thread is translated to an envelope | `pages/members/MembersPage.tsx` (unchanged in M0) | `src/kiro_crew/member_inbox.py`, `src/kiro_crew/member_scheduler.py`, `src/kiro_crew/member_peer.py`, `member_wake.py`, `handlers/member_inbox.py`, `chat_handlers.py` (`_member_inbox_intake`), `session_control.py` (`_inbox_model_shim`) | `POST /api/chat` (flagged member slot → `{inbox: true}`), `POST /api/member-inbox/outbox`, `POST /api/member-inbox/peer-send` (strict-internal, MCP tools `outbox_send` / `peer_send`) |
