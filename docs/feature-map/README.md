...
| Files in chat | Browse, attach and upload workspace files; search the rail by file NAME or by file CONTENTS, including the text inside Word/PowerPoint/Excel documents | Chat right panel → Files tab, Name/Content toggle below the search field; composer attach | `pages/chat/FileBrowserRail.tsx`, `pages/chat/FilesHomePanel.tsx`, `api/fileGrep.ts` | `handlers/files.py` | `GET /api/file-read`, `GET /api/file-grep`, `POST /api/upload`, `POST /api/upload/file` |
