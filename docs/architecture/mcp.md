...
The organization prototype adds `org_inbox`, `org_start_task`, `org_hire`, `org_assign`,
`org_message`, `org_report`, `org_review` and `org_retry` to the existing
`kirocrew-work` server. Schemas accept task and recipient identifiers, never a
caller, session or memory identity. Each call uses its current verified private
session to reach `/api/organization-agent`; a shared secret and a selected name
cannot confer membership. `/api/organization` is an owner-only dashboard
surface, unavailable to internal and app callers. Organization roles select
individual tools from this server and `kirocrew-core` in a dedicated agent
specification. Per-caller state remains outside the MCP module.
Organization gateway errors from both reads and mutations return `Error:`
followed by their redacted JSON payload, retaining the code, field and diagnostic
details. The shared MCP wrapper records these calls as `failed`; successful
responses remain JSON and are audited as `completed`.
`org_start_task` is available to every organization role. It accepts only title
and acceptance, records the owner as sender and the verified caller as recipient,
and requires a live owner-chat grant held by the gateway's slot. Its idempotency
key is generated by the gateway, not accepted from the tool. Ordinary delegation
still requires an active parent assigned to the delegating manager.
The internal call-site coverage guard scans `organization_tools.py` alongside
the existing MCP modules, checking that its transport paths remain allowlisted.
