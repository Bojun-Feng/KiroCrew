...
| Module | Carries |
|---|---|
| `operation.py` | `OperationDescriptor` — what an operation *is*: `operation_id`, `service_id`, `operation_kind`, `effect`, and the new `credential_mode`. The first four enums are copied VERBATIM from the closed sets in [connector-capability-manifest.md](connector-capability-manifest.md); that spec owns them, and this seam neither invents a value nor drops one. |
| `context.py` | `OperationContext` — the per-call references: `binding_ref`, `tenant_ref`, `subject_ref`, `deadline`. Every field is a REFERENCE; a context never carries a token value or any credential plaintext, following directly from the credential boundary above. `deadline` is an absolute POSIX-seconds UTC cutoff, not a relative budget. |
...
- **Axis B — credential mode** answers *what credential this operation
  ...
  `service_to_service`. It is the new `credential_mode` enum on
  `control_plane/operation.py`.
...
`credential_mode` off the operation descriptor directly and never computes it
