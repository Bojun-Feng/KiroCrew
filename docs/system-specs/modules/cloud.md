...
| `iam.py` | Least-privilege launcher policy generator (applied by the user, never by Kiro Crew) + read-only reachability check + the **content-fixed instance permissions-boundary document** (`boundary_policy_document`/`boundary_arn`) and its constants (`BOUNDARY_NAME`). The launcher document never carries `InvokeGateway` / `GetWorkloadAccessToken*` / inspect verbs (`GetGateway`, `ListGatewayTargets`, `GetGatewayTarget`). A labeled sibling `agentcore_instance_policy_document(posture)` is the instance-role fragment (`workload` or `login`) and includes a read-only inspect SID on `gateway/*` so a later catalog surface can inspect an operator-pasted Gateway. Workload Allow is `GetWorkloadAccessToken` only. The instance GRANT is exact: the directory plus `workload-identity/kirocrew` (a CloudFormation launch emits its own exact `kirocrew-<tag>` grant via `!GetAtt CrewWorkloadIdentity.WorkloadIdentityArn`) — never `kirocrew-*` in a grant, since two tagged identities plus that wildcard would let one crew mint a sibling bearer. The shared CEILING (`AgentCoreUnionCeiling`) and the launcher's control-plane verbs admit `workload-identity/kirocrew-*` as well, because every tagged launch creates `kirocrew-<tag>` and a ceiling that excluded it would deny both its creation and its token vend; a ceiling does not grant. What keeps a leaked launcher credential from minting a sibling token under that ceiling is two-fold: `IamCreateRoleWithBoundary` admits only the original boundary (a posture launch runs under admin credentials), and `DenyLauncherOnAgentCoreBoundedRoles` explicitly denies the launcher `PutRolePolicy` / `AttachRolePolicy` / `PassRole` / `TagRole` / `UntagRole` on any role the template tagged `kirocrew:boundary=agentcore` (applied at CreateRole, alongside `kirocrew:managed`; the value is derived from the `PermissionsBoundaryArn` the role is created under — condition `IsAgentCoreBoundary` — never from `AgentCorePosture`, so a successor-bounded role launched with posture `none` is still classified `agentcore`), so an existing posture role's grant cannot be rewritten toward a sibling identity and re-passed, and the classifying tag cannot be stripped. `InvokeGateway` is omitted until the operator scopes it to the configured Gateway (`kirocrew-*` would let one crew invoke a sibling). `GetWorkloadAccessTokenForUserId` stays on the login Deny. `SynchronizeGatewayTargets` is not granted. `AGENTCORE_BOUNDARY_NAME` (`kirocrew-ec2-boundary-agentcore`) is the successor union ceiling plus `AgentCoreInspectCeiling` and does not include `InvokeGateway`. `IamCreateRoleWithBoundary` matches only the original `BOUNDARY_NAME` so a leaked launcher credential cannot CreateRole a successor-capped role and mint a sibling `kirocrew-*` token via `PutRolePolicy`. |
...
| `templates/kirocrew-ec2.yaml` | The CloudFormation stack. `AgentCorePosture=none\|workload\|login` (default `none`) optionally creates `AWS::BedrockAgentCore::WorkloadIdentity` named `kirocrew-<StackTag>` and attaches the matching instance-role grant. `AgentCoreGatewayUrl` is an existing Gateway MCP URL written into the instance unit. A non-`none` posture installs `kirocrew[agentcore]` via `install.sh --agentcore`. The default (`none`) launch does not deploy this file as-is: CloudFormation validates every resource type against the region's registry before it evaluates `Condition`s, so a conditional BedrockAgentCore resource would fail change-set creation in a region without the service. `ec2.base_template_text` strips the `# >>> agentcore-only` … `# <<< agentcore-only` blocks (the identity resource, the two posture policies, the posture conditions, the two outputs) and `ec2.staged_template` stages the variant a launch deploys — the base variant for `none`, the full template for a posture launch — for exactly one deploy: under `<config_dir>/run/` (the runtime exec dir the agent sandbox denies read and write on), in a fresh 0700 subdir with an unpredictable name, written `O_NOFOLLOW|O_EXCL`, and removed once the deployment child exits — whatever `run_aws` reads executes with the launcher's authority, so no launch hands CloudFormation a fixed path an in-sandbox agent could pre-write (the dry run shows a placeholder naming the variant, not a path). That includes the bundled file itself: the package directory is not a tree the sandbox masks, and in a personal install it is writable by the sandbox's own uid, so a posture launch deploys a snapshot read once in the gateway, never the bundled path. The AgentCore parameters stay in both (the launcher always passes them). The dashboard launch is the `none` launch: its handler 400-refuses a posture or Gateway URL and the launch job carries neither field. CDK operators create the same identity resource via `aws_bedrockagentcore.WorkloadIdentity`. |
...
an incomplete upload cannot pin a thread forever. A connection holds
one of those handlers from accept, before it has authenticated, and the
sandbox shares the loopback -- so until the bound token verifies it has
only ``PROXY_PREAUTH_DEADLINE_SECS`` of wall clock (a timer, not a
per-recv timeout, so trickled bytes do not extend it) to deliver an
authenticating request line and headers; the 300 s body/upstream budget
applies only after. Otherwise 16 idle sockets would deny every
session's Gateway access for five minutes at a time. Catalog
