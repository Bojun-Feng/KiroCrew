...
the record but never silently diverges from it: every mirrored record (the `user_dm`
envelope, the `reply` row) is WRITTEN with `view: pending` in its own durable write and
stamped `view: shown` only after its thread row is confirmed on disk, so a crash at any
later point -- a failed save, or the process dying before the stamp -- leaves a row the
recovery scan finds; the next intake or wake restores every pending row to the thread
(`reconcile_thread_view`, re-saving the live window when it still holds the row,
appending when it does not; an append to a thread that is not loaded goes through the
history log's `append_if_absent` with the envelope / outbox id as the row's `mid`, so a
restore of a row whose first append landed but whose stamp did not writes nothing) before
it processes anything. Every envelope read-modify-write in both stores (`attempts`, `refs`
stamps, the pending → acked / dead-letter transitions) runs under one per-member re-entrant
lock (`member_store_lock`), so a wake's `mark_attempt` and an intake's view stamp on the same
file cannot drop each other's write. The rollback fold's `inject` row is
...
dead-letter notice and owner notification at the ceiling. A wake slot whose `close_slot`
fails (its history save raised) is evicted by the runner itself (`_evict_wake_slot`:
registry entry, live task, provider session), because the close's restore-the-slot
behaviour protects a person's notes but a wake is never revisited under its fresh key
and would otherwise leak one slot and one provider subprocess per interval under a
persistent disk fault. A wake that ends with envelopes still pending is retried by
...
acked without a second turn, so the owner never reads the same answer twice — and this
check runs BEFORE the attempt count moves or the dead-letter ceiling is applied, so an
answered envelope sitting at the ceiling is finished work, never dead-lettered and
reported as "crashed"; an early
...
that anchor because those rows are the unattended-send count, and every outbox row newer
than the read marker because those are the projection's unread rows (retention gives way to
the record: a member that talks more than the person reads holds more than 500 rows until the
thread is opened); pending rows are never pruned, so the hot paths (`last_user_dm_at`,
`peer_sends_since`, `recent_exchange`) stay bounded once read.
...
**Wake context.** The wake slot is execution bookkeeping — an ephemeral dashboard slot
with its own key and its own (discarded) transcript — but the MODEL CONTEXT is the
member's one durable provider conversation (`members.<slug>.wake_context: resume`, the
default): `member_wake._bind_wake_conversation` seeds the wake's session key with the
member's persisted native session id (`member-inbox/<slug>/conversation.json`; the first
time, the live thread's own resumable sid, adopted so flipping the flag on loses nothing,
with the thread's idle provider retired so one process holds the session — a wake that
finds the thread's own turn still running, the one in flight when the flag flipped, defers
instead: envelopes stay pending with no attempt counted and the scheduler's retry picks
them up once the turn has ended, so the retire never aborts live output), the provider
`session/load`s it, and the wake appends one turn — the drained envelope batch is that
turn's user message — so the model carries the same conversational memory a live member
thread has; the ledger and the recent-exchange window ride along as structured memory, not
the only memory, and growth is the provider's ordinary compaction of MODEL context (inbox
and outbox rows are never compacted by it). After the turn `_settle_wake_conversation`
records the sid the turn ran in and drops the wake key's own map entry, so the map does
not grow one row per wake. The conversation is ONLY the model context — not the delivery
channel (the inbox), not the record (the outbox), not the identity (the binding) — so a
conversation that is lost or refused costs continuity, never a message: a seed the
session map prunes (native session gone) is a reset known BEFORE the turn, a provider that
refuses the load is detected after it (`_provider_resumed`: the session map now holds a sid
other than the seeded one, or the ACP session client — `provider.client`, the outer provider
carries no such flag — reports `resumed is False`); either way the wake runs in a
new conversation seeded by the prompt's ledger + recent exchange, and a `system` outbox
row (`refs.wake_context_reset`, the reason) tells the human in the projection. `fresh` is
today's ephemeral wake (ledger + exchange window only) for pure-patrol members. Failure
semantics are unchanged: a non-`end_turn` stop is a failed wake, envelopes unacked and
redelivered. When the flag goes off, the thread's next message adopts the wakes'
conversation back onto the thread's session (`adopt_wake_conversation_into_thread`) and
clears the record; while a wake is still running for an unflagged member, the intake refuses
the send with a retryable `member_wake_running` 409 instead of running an ordinary turn whose
provider conversation the later adoption would replace. Token cost on `resume` is a live member turn's, not the cheaper
ephemeral one — the trade the owner chose over context loss. The wake slot keeps the
...
once cannot move it backwards); the same re-entrant lock (`member_marker_lock`) covers
`OutboxStore.append` (mint + durable write) and the read endpoint's projection-read +
marker-write as one transaction, so a row minted while a `/read` decides cannot be
timestamped before the marker and land behind it uncounted. The projection's served window — the newest
...
is I/O-free, deciding only from the slot's mode and creator. A turn that did NOT
complete and that the runner is about to recover (an empty or transient-failed response,
a refused tool before any answer, a stall — a synthetic recovery item is queued) is not
terminal and is not reported; the recovery turn reports its own end. A turn that DID
complete is reported whatever recovery follows it — a Stop-hook continuation, or a
refusal recovery queued after the model already answered, runs as a further turn with its
own tail and reports only that — so the snapshot's outcome, not the recovery's prefix, is
what decides. Then `member_wake.report_worker_turn`, off the loop, reads the creator's `inbox_model` flag
...
THIS turn (every assistant row since the last turn-opening row — a `user`, `nudge` or
`subagent` row, or an `inject` row the dispatch stamped `meta.injectKind` (cron,
recovery, synthesis); a runner NOTICE such as the Stop-hook halt card is an `inject` row
without that stamp and is not a boundary — joined in order — a
tool-using turn writes a segment before and after each tool call), bounded to `WORKER_REPORT_MAX_CHARS`,
`from session:<key>`, `refs.outcome: "ok"`; a turn that appended an error row, ended
with no new assistant text, or stopped without finishing (a non-`end_turn` stop reason:
timeout, cancel, stall, token cap -- the wake runner's own `_turn_cut_short` rule, with the
partial text carried and labelled as partial) is reported as `refs.outcome: "failed"`,
never with a previous turn's reply. A member's own thread or wake never
...
its `wake_timer` cadence; `members.<slug>.wake_context` (`resume` | `fresh`, default
`resume`) chooses whether wakes continue one durable conversation or start ephemeral
(anything but the literal `fresh` reads as `resume`); `members.<slug>.peer_dm.{accept,send}` (bool, default
