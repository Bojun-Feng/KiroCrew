...
def _grep_rg(root: str, query: str, deadline: float) -> tuple[list[dict], bool] | None:
    ...
    and an error exit (>1; 1 is "no matches") are all "no verdict", and an empty
    list would tell the user "no matches" about a search that never ran. A
    TIMEOUT is NOT a fallback: the deadline is shared, so the python pass would
