...
async def api_member_read(request: web.Request) -> web.Response:
    ...
    def _mark() -> dict | web.Response:
        ...
        # One transaction with the member's outbox appends (same lock inside
        # `OutboxStore.append`): a row minted while this read decides cannot
        # land behind the marker it writes, so nothing unread is ever counted
        # as read. Off the loop, like the rest of this closure.
        with inbox_mod.member_marker_lock(slug):
            rows = inbox_mod.projection(slug)
            served = inbox_mod.served_window(rows, inbox_mod.read_marker(slug), _PROJECTION_LIMIT)
            # Only a row the projection endpoint would have served may become
            # the marker: the client cannot name a row it was never shown.
            ...
            unread = inbox_mod.unread_count(slug, rows)
        return {"ok": True, "slug": slug, "marker": marker, "unread": unread}
