...
def _read_managed_tool_policy_sync(agents_dir: Path, agent_name: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        if config is None:
            # ``<name>.json`` then ``<name>.md``, the order every other
            # direct-filename fallback uses (see ``kas_agents.load_agent_spec``).
            present = [p for p in agent_spec_candidates(agents_dir, agent_name) if p.is_file()]
            if len(present) != 1:
                # Absent, or a JSON and a markdown twin: which one is live is
                # undefined, so no policy is the honest answer for both.
                return None
            agent_path = present[0]
            config = parse_agent_spec_text(agent_path.read_text(encoding="utf-8"), agent_path)
