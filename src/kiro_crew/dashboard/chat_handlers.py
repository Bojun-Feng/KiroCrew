...
async def _member_inbox_intake(
    ...
    try:
        ...
        if not await asyncio.to_thread(inbox_model_enabled, slug):
            ...
            if member_wake_running(slug):
                # The flag went off while a wake was still running. That wake
                # holds the member's conversation and may still claim envelopes;
                # running an ordinary turn now would start a SECOND provider
                # conversation that the later adoption then replaces (its
                # context lost), and folding now would race the wake's claims.
                # Refuse, retryable: the wake is bounded by its wall clock.
                return web.json_response(
                    {
                        "error": "this member's last wake is still finishing; retry shortly",
                        "code": "member_wake_running",
                        "retry_after": 15,
                    },
                    status=409,
                )
            # Rollback: the thread runs turns again, and it should run them in
            # the conversation the wakes were having, not in whatever it had
            # before the flag went on. Best-effort; the thread's own prior
            # session is the fallback.
            from kiro_crew.dashboard.member_wake import adopt_wake_conversation_into_thread

            try:
                await adopt_wake_conversation_into_thread(state, slug, slot.key)
            except Exception:  # noqa: BLE001
                logger.warning(
                    "member inbox: conversation adoption failed for %s", slug, exc_info=True
                )
        ...
        from kiro_crew.member_inbox import VIEW_BORN_PENDING
        ...
        # Born `view=pending` in the same durable write: a crash anywhere before
        # the thread row is confirmed leaves an envelope the recovery scan finds.
    ...
    # restart does not empty the visible conversation. The envelope was born
    # `view=pending` and is stamped `shown` only after a confirmed save; a save
    # that fails leaves it for the next intake or wake to restore
    # (`reconcile_thread_view`) before it processes anything else. The scheduler
    # is told LAST: a wake woken before this bookkeeping finished would rewrite
    # the envelope (`mark_attempt`) under the stamp and could ack it before the
    # stamp landed.
    if await _persist_thread_row(state, slot, slot.key):
