...
async def _start_next_queued_turn(state: DashboardState, slot: _ChatSlot) -> bool:
    ...
    # The surface the queued message ARRIVED from. A merge run never crosses an
    # ingress boundary (``_dequeue_next_message``), so every entry in a batch
    # shares one value and ``all`` is exact rather than conservative: a batch
    # of Slack-routed messages is Slack ingress and is not echoed back into the
    # thread; a batch of dashboard-typed ones is echoed.
    ingress = (
        SLACK_NAMESPACE
