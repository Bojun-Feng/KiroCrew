...
def _report_worker_turn_end(state: DashboardState, slot: _ChatSlot) -> None:
    ...
    if worker_turn is None:
        return
    # A turn that did NOT complete and that the runner is about to recover --
    # an empty or transient-failed response, a refused tool before any answer,
    # a stall -- is not terminal: reporting NOW would file a "failed" report and
    # wake the member before the recovery runs. The recovery turn ends through
    # this same hook and reports its own outcome; if it exhausts, that end is
    # the failed one. A turn that DID complete is reported whatever recovery
    # follows it: a Stop-hook continuation, or a refusal recovery queued after
    # the model already answered, runs as a further turn with its own tail, and
    # `_turn_outcome` reads only that turn's rows -- deferring here would drop
    # the finished reply from the member's inbox for good. The outcome, not the
    # recovery's prefix, is what tells the two apart.
    if worker_turn.outcome != "ok" and any(
        is_synthetic_recovery_item(item) for item in (slot._queue or ())
    ):
        return
