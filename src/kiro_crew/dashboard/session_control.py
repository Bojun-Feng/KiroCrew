...
async def _inbox_model_shim(
    ...
    *,
    ...
    # Redaction can GROW a body (a token becomes a longer placeholder), so a
    # limit-sized message can leave the send-size gate above and still exceed
    # the envelope cap. Checked here, as a typed refusal, rather than letting
    # `make_envelope` raise a bare `InboxError` into a 500.
