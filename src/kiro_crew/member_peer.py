...
def peer_dm_enabled() -> bool:
    ...
    if "member_peer_dm" in degraded:
        # The loader normalized a malformed section to `enabled: false` and
        # recorded it (config.loader._fail_closed_member_peer_dm): the value
        # in hand is the loader's, not the operator's, so this is a degraded
        # config, not a switch the operator turned off.
        raise _Degraded("member_peer_dm section is malformed")
