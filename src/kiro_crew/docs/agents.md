...
- `<name>.json` and `<name>.md` side by side is the same agent twice; Kiro Crew refuses to pick one. Keep one file per name.
...
| Runs on the `kiro` (kiro-cli) backend | yes | no: kiro-cli loads JSON only, so the session is refused before spawn with a message naming the file. Switch `agent.acp_backend` to `kas` or add a JSON spec. |
