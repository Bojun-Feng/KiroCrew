...
#: The member's ONE durable provider conversation (``members.<slug>.wake_context:
#: resume``): the native session id every wake appends a turn to, persisted here
#: so it survives a gateway restart and outlives every ephemeral wake slot. Only
#: the MODEL CONTEXT lives in that conversation -- delivery is the inbox, the
#: record is the outbox, identity is the binding -- so a conversation that is
#: lost or refused costs continuity, never a message.
CONVERSATION_FILE_NAME = "conversation.json"
#: ``members.<slug>.wake_context`` values. ``resume`` (default): one durable
#: conversation per member, each wake one more turn in it -- the model keeps the
#: same conversational memory a live member thread has. ``fresh``: today's
#: ephemeral wake (ledger + rolling exchange window only), for pure-patrol members
#: and the automatic fallback when a resume fails.
WAKE_CONTEXT_RESUME = "resume"
WAKE_CONTEXT_FRESH = "fresh"
...
#: ``refs.view`` of a durable record whose THREAD ROW (the conversational view)
#: the person may not have on disk yet. The store is the record and the
#: transcript is the view, but a view that silently diverges from the record is
#: a message the person never sees. Every mirrored record (a ``user_dm``
#: envelope, a ``reply`` row) is WRITTEN with ``view=pending`` -- one durable
#: write, so a crash at any later point leaves a row the recovery scan finds --
#: and stamped ``view=shown`` only after its transcript row is confirmed on
#: disk. ``member_wake.reconcile_thread_view`` re-appends every pending row
#: before the next intake or wake processes anything (re-saving the live window
#: when it still holds the row, so the restore is idempotent).
VIEW_REF = "view"
VIEW_PENDING = "pending"
VIEW_SHOWN = "shown"
#: The refs a freshly written, mirrored record is born with.
VIEW_BORN_PENDING = {VIEW_REF: VIEW_PENDING}
...
def wake_context_mode(slug: str) -> str:
    """``members.<slug>.wake_context`` -- ``resume`` unless the operator wrote ``fresh``.

    Anything else (absent, malformed) is the default: continuity is the safe
    reading of an unset knob, and a typo must not silently turn a member into
    an amnesiac.
    """
    raw = member_inbox_setting(slug, "wake_context", WAKE_CONTEXT_RESUME)
    return WAKE_CONTEXT_FRESH if raw == WAKE_CONTEXT_FRESH else WAKE_CONTEXT_RESUME


def read_conversation(slug: str) -> dict[str, str] | None:
    """The member's persisted conversation ``{"sid", "provider", "cwd"}``, or ``None``."""
    path = member_store_dir(slug) / CONVERSATION_FILE_NAME
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, ValueError):
        return None
    if not isinstance(raw, dict) or not str(raw.get("sid") or ""):
        return None
    return {
        "sid": str(raw["sid"]),
...
def write_conversation(slug: str, sid: str, *, provider: str = "", cwd: str = "") -> None:
    """Durably record the member's conversation (atomic replace, like every store write)."""
    if not sid:
        return
    root = member_store_dir(slug)
    root.mkdir(parents=True, exist_ok=True)
    atomic_write(
        root / CONVERSATION_FILE_NAME,
        json.dumps(
            {"sid": sid, "provider": provider, "cwd": cwd, "updated_at": now_iso()},
            ensure_ascii=False,
        ),
        fsync=True,
    )
    fsync_dir(root, best_effort=True)


def clear_conversation(slug: str) -> bool:
    """Forget the member's persisted conversation; ``True`` when there was one."""
    return _unlink_quiet(member_store_dir(slug) / CONVERSATION_FILE_NAME) == 1
...
#: One lock per member around every envelope read-modify-write in BOTH stores
#: (`attempts`, `refs` stamps, state transitions). Each write is atomic on its
#: own, but two RMWs on the same file from different threads -- a wake's
#: `mark_attempt` racing an intake's `view` stamp -- would otherwise land the
#: later read's copy and drop the earlier write (an attempt increment lost is
#: one more execution past the redelivery ceiling). Re-entrant, so a locked
#: caller may call a locked helper. Filesystem-bound callers only.
_store_locks: dict[str, threading.RLock] = {}
_store_locks_guard = threading.Lock()


def member_store_lock(slug: str) -> threading.RLock:
    with _store_locks_guard:
        lock = _store_locks.get(slug)
        if lock is None:
            lock = _store_locks[slug] = threading.RLock()
        return lock
    ...
    def ack(self, env_ids: list[str]) -> int:
        ...
        for env_id in env_ids:
            try:
                ...
                continue
            with member_store_lock(self.slug):
                moved += self._ack_one(path)
        return moved

    def _ack_one(self, path: Path) -> int:
        """One envelope's pending -> acked transition; caller holds the store lock."""
        if not path.exists():
            return 0
