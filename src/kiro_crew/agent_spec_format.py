...
def iter_agent_spec_files(directory: Path, *, ordered: bool = True) -> list[Path]:
    """Every spec file in *directory*, both forms.

    Propagates ``OSError`` from the directory walk exactly as ``Path.glob``
    does, so a caller that already handles the JSON-only glob's failure handles
    this one unchanged. *ordered* sorts by full name so ``foo.json`` precedes
    ``foo.md`` and the order is stable across platforms; ``ordered=False`` keeps
    the directory's native order, JSON entries first, for the first-match
    resolvers that scan on the event loop and stop at the first hit.
    """
    files: list[Path] = []
    for suffix in AGENT_SPEC_SUFFIXES:
...
def agent_spec_candidates(directory: Path, name: str) -> list[Path]:
    ...
    JSON first: the two never coexist legitimately, and a caller resolving one
    treats the pair as an ambiguity, but a fixed order keeps its message stable.
    """
...
def parse_markdown_spec(text: str) -> dict[str, Any]:
    ...
    valid YAML, or it is valid YAML that is not a mapping. ``yaml.safe_load``
    only: the agents directory is user-writable and shared with other tools.
    """
