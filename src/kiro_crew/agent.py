...
def agent_spec_path(name: str, *, agents_dir: Path | None = None) -> Path | None:
    ...
    *name* is validated against the shared agent-name grammar BEFORE it reaches
    ...
    if len(fallbacks) > 1:
        # ``<name>.json`` AND ``<name>.md``, neither declaring this name. The
        # runtime resolvers match the stem of either, so which is live is as
        # undefined as two declared matches -- refuse for the same reason.
        raise ValueError(
            f"{len(fallbacks)} spec files carry the filename {name!r}: "
            f"{', '.join(repr(str(p)) for p in fallbacks)}. A JSON and a markdown spec "
            f"with the same stem are the same agent twice -- remove or rename one."
        )
    return fallbacks[0] if fallbacks else None


def markdown_spec_for_agent(agent: str, work_dir: str | Path | None = None) -> Path | None:
    """The markdown spec *agent* would run from, or ``None`` when it is JSON or absent.

    Resolves in kiro-cli's own order -- the project checkout's ``.kiro/agents``
    first, then the user directory -- so the answer names the file the backend
    would actually load. Only the KAS backend reads the markdown form; kiro-cli
    discovers ``*.json`` alone and answers a ``set_mode`` for a markdown-only
    agent with ``Mode not found``, so the kiro-cli harness asks this before
    spawning and refuses with a message that names the file instead.
        ...
        if work_dir:
            for spec in project_agent_files(work_dir):
                if project_agent_name(spec) == agent:
                    return spec if is_markdown_spec(spec) else None
        path = agent_spec_path(agent)
    except (OSError, ValueError):
        return None
    if path is not None and is_markdown_spec(path):
        return path
    return None
...
def _conflicting_spec_for(name: str, chosen: Path, agents_dir: Path) -> Path | None:
    ...
    """
    for direct in agent_spec_candidates(agents_dir, name):
