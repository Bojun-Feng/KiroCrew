...
class TestInboxStore:
    ...
    def test_concurrent_attempt_and_stamp_writes_are_serialized(self):
        """`mark_attempt` (a wake starting) and `mark` (a view stamp) read-modify-write
        the same file from different threads. Under the per-member store lock
        neither write is lost: every attempt increment survives -- a lost one is
        an execution past the redelivery ceiling -- and so does the stamp."""
        import threading
        ...
        def stamps():
            try:
                for i in range(40):
                    store.mark(env.id, **{mi.VIEW_REF: f"s{i}"})
            except BaseException as exc:  # noqa: BLE001
                errors.append(exc)

        threads = [threading.Thread(target=attempts), threading.Thread(target=stamps)]
        for th in threads:
            th.start()
        for th in threads:
            th.join()
        assert errors == []
        (final,) = store.pending()
        assert final.attempts == 40 and final.refs[mi.VIEW_REF] == "s39"
        assert isinstance(mi.member_store_lock("radar"), type(threading.RLock()))
    ...
    def test_compaction_prunes_oldest_but_keeps_the_budget_anchor(self, monkeypatch):
        ...
        replies = [out.append(kind=mi.OUTBOX_REPLY_KIND, body=f"r{i}") for i in range(4)]
        # the person has read everything: retention applies to the outbox in full
        mi.write_read_marker("radar", {"id": replies[-1].id, "created_at": replies[-1].created_at})
    ...
    def test_compaction_never_prunes_rows_the_person_has_not_read(self, monkeypatch):
        """Retention gives way to the record: an outbox row newer than the read
        marker is an unread row the badge promises, and a hard delete would drop
        it (and the count) silently. Only rows at or before the marker are
        candidates; nothing read means nothing pruned."""
        monkeypatch.setattr(mi, "RETAIN_OUTBOX", 2)
        out = OutboxStore("radar")
        rows = [out.append(kind=mi.OUTBOX_REPLY_KIND, body=f"r{i}") for i in range(6)]
        assert mi.compact_member("radar") == 0 and len(out.rows()) == 6  # nothing read yet
        assert mi.unread_count("radar") == 6
        # read up to the third row: the three read rows are candidates, keep 2 of
        # the total budget... the 4 unread survive regardless of RETAIN_OUTBOX
        mi.write_read_marker("radar", {"id": rows[2].id, "created_at": rows[2].created_at})
        assert mi.compact_member("radar") == 3
        assert [e.id for e in out.rows()] == [e.id for e in rows[3:]]
        assert mi.unread_count("radar") == 3  # every unread reply still counted
    ...
    def test_compaction_never_prunes_peer_sends_that_count_against_the_budget(self, monkeypatch):
        ...
        last = None
        for i in range(8):
            last = out.append(kind=mi.OUTBOX_REPLY_KIND, body=f"r{i}")
        assert last is not None
        mi.write_read_marker("radar", {"id": last.id, "created_at": last.created_at})
        removed = mi.compact_member("radar")
        rows = out.rows()
...
class TestProjectionEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_marker_write_and_outbox_append_are_one_transaction(self):
        """An outbox row minted while `/read` decides cannot land behind the marker
        it writes: the append and the projection-read + marker-write run under the
        member's lock, so a row is either seen by the read (and may be marked) or
        timestamped after the marker (and counts as unread)."""
        import threading

        from aiohttp.test_utils import TestClient, TestServer

        from kiro_crew.member_inbox import member_marker_lock, unread_count

        out = OutboxStore("radar")
        first = out.append(kind=mi.OUTBOX_REPLY_KIND, body="first")
        # Hold the lock as an in-flight append would; the read must wait for it.
        lock = member_marker_lock("radar")
        lock.acquire()
        ...
        async with TestClient(TestServer(_projection_app())) as client:
            ...
            async def read():
                resp = await client.post("/api/members/radar/read", json={"last_read_id": first.id})
                outcome["status"] = resp.status
                outcome["body"] = await resp.json()

            task = asyncio.get_running_loop().create_task(read())
            await asyncio.sleep(0.2)
            assert "status" not in outcome  # blocked behind the in-flight append
            # The in-flight append completes (re-entrant for its own thread: this
            # is the same lock `append` takes) and releases; its row is newer
            # than `first`.
            out.append(kind=mi.OUTBOX_REPLY_KIND, body="late")
            lock.release()
            await asyncio.wait_for(task, 5)
        assert outcome["status"] == 200
        # The marker points at `first`; `late` was minted after it and stays unread.
        assert outcome["body"]["marker"]["last_read_id"] == first.id
        assert outcome["body"]["unread"] == 1 and unread_count("radar") == 1
...
class TestWorkerReport:
    ...
    def test_reply_is_redacted_before_the_tail_cut(self):
        """A credential whose anchor sits just before the cut: redacting the
        truncated tail would see a bare token with no `Authorization:` in front
        of it and let it through. The whole reply is redacted first."""
        token = "Q7x9" * 12  # opaque: only the `Authorization: Bearer` anchor identifies it
        # The tail cut lands INSIDE the anchor: the last WORKER_REPORT_MAX_CHARS
        # characters start at "earer <token>", so the whole token survives the cut
        # while the word the redactor keys on does not.
        tail_after_token = self.mw.WORKER_REPORT_MAX_CHARS - len(f"earer {token}\n")
        reply = "x" * 500 + f"Authorization: Bearer {token}\n" + "y" * tail_after_token
        slot = _WorkerSlot("chat-9-10", created_by="member-fixer", reply=reply)
        self._report(slot)
        (env,) = InboxStore("fixer").pending()
        assert token not in env.body
        assert len(env.body) <= self.mw.WORKER_REPORT_MAX_CHARS + len("[Triage #42]\n")
    ...
    def test_a_trailing_notice_inject_row_is_not_a_turn_boundary(self):
        """The Stop-hook halt card (and any other runner NOTICE) is an `inject` row
        appended as the last row of a turn that completed. Only a DISPATCHED
        inject -- one stamped `meta.injectKind` by the dispatch -- opens a turn;
        a notice must not make the report read "ended without a reply"."""
        from kiro_crew.dashboard.state import HOOK_HALTED_RECOVERY_PREFIX

        slot = _WorkerSlot("chat-8-6", created_by="member-radar", reply="PR opened.")
        slot.messages.append(
            {
                "role": "inject",
                "content": f"{HOOK_HALTED_RECOVERY_PREFIX} #3\nA Stop hook asked to continue...",
                "cls": "msg msg-inject",
            }
        )
        env_id = self._report(slot)
        ...
        # A DISPATCHED inject (cron notification, recovery, synthesis) still opens
        # a turn: the previous reply does not ride along.
        slot.messages += [
            {
                "role": "inject",
                "content": "[Cron notification] nightly",
                "cls": '{"cronLabel": "nightly"}',
                "meta": {"injectKind": "cron", "cronLabel": "nightly"},
            },
            {"role": "assistant", "content": "nightly done"},
        ]
        env_id2 = self._report(slot)
        (env2,) = [e for e in InboxStore("radar").pending() if e.id == env_id2]
        assert env2.body.endswith("nightly done") and "PR opened." not in env2.body

    @pytest.mark.parametrize("stop", ["timeout", "cancelled", "max_tokens"])
    def test_a_turn_cut_short_by_the_provider_is_reported_as_failed(self, stop):
        """Partial text plus a non-clean stop reason is the wake runner's own
        "not a result" case; the worker report applies the same rule instead of
        writing a durable `ok` the creator would act on."""
        slot = _WorkerSlot("chat-8-4", created_by="member-radar", reply="half an answ")
        slot._last_stop_reason = stop
        env_id = self._report(slot)
        (env,) = [e for e in InboxStore("radar").pending() if e.id == env_id]
        assert env.refs["outcome"] == "failed"
        assert stop in env.body and "stopped before finishing" in env.body
        assert "half an answ" in env.body  # the partial text is carried, labelled as such
        # a clean end (or no reason at all) is still a result
        for clean in ("end_turn", ""):
            ok_slot = _WorkerSlot("chat-8-5", created_by="member-radar", reply="whole answer")
            ok_slot._last_stop_reason = clean
            turn = self.mw.snapshot_worker_turn(ok_slot)
    ...
    def _turn_end_harness(self, monkeypatch):
        """`_report_worker_turn_end` with the writer stubbed: returns the list of
        outcomes it decided to report, in order."""
        from kiro_crew.dashboard import chat_runner as cr

        reported: list[str] = []
        # the writer is what a deferral skips: record what reaches it, not the snapshot
        monkeypatch.setattr(
            self.mw, "report_worker_turn", lambda state, turn: reported.append(turn.outcome)
        )
        ...
        async def end(slot):
            cr._report_worker_turn_end(bg, slot)
            if bg._background_tasks:
                await asyncio.gather(*list(bg._background_tasks))  # the fire-and-forget writer
            return list(reported)

        return end

    @pytest.mark.asyncio
    async def test_a_turn_awaiting_a_synthetic_recovery_is_not_reported_yet(self, monkeypatch):
        ...
        end = self._turn_end_harness(monkeypatch)
        slot = _WorkerSlot("chat-r-1", created_by="member-radar", reply="")
        slot._queue = [{"content": "continue", "kind": SYNTHETIC_RECOVERY_KIND}]
        assert await end(slot) == []  # deferred: the recovery turn reports
        slot._queue = [{"content": "a real follow-up", "kind": ""}]
        assert await end(slot) == ["failed"]  # an ordinary queued successor does not defer

    @pytest.mark.asyncio
    @pytest.mark.parametrize(
        "prefix_name", ["HOOK_CONTINUATION_RECOVERY_PREFIX", "REFUSAL_RECOVERY_PREFIX"]
    )
    async def test_a_completed_turn_is_reported_whatever_recovery_follows_it(
        self, monkeypatch, prefix_name
    ):
        """A Stop hook that blocks, or a tool refusal AFTER the model already
        answered, queues a synthetic item after a turn that COMPLETED. That is not
        a retry: the follow-up runs as a further turn with its own tail, so this
        turn's finished reply must be reported now or it never reaches the
        member's inbox. The same item queued after a turn with NO answer is a
        retry and defers."""
        ...
        queued = [
            {"content": f"{getattr(st, prefix_name)}\ncarry on", "kind": SYNTHETIC_RECOVERY_KIND}
        ]
        done = _WorkerSlot("chat-r-2", created_by="member-radar", reply="PR opened.")
        done._queue = queued
        assert await end(done) == ["ok"]  # the completed answer is reported first
        unanswered = _WorkerSlot("chat-r-3", created_by="member-radar", reply="")
        unanswered._queue = queued
        assert await end(unanswered) == ["ok"]  # nothing new: the recovery re-runs and reports
...
class _Log:
    ...
    def append_if_absent(self, key, role, content, *, mid=None, **_):
        # the real one dedups a body-equal row carrying the same meta.mid
        if mid and mid in self.mids:
            return False
        if mid:
...
def _provider_like(*, resumed: bool, sid: str) -> Any:
    """The shape the real ACP provider has: the OUTER object carries no
    `resumed` flag; it lives on the inner session client (`provider.client`).
    A fake with the flag on the outer object let a wrong read pass the suite."""
    return SimpleNamespace(client=SimpleNamespace(resumed=resumed), sid=sid)


class _Sessions:
    """A session manager reduced to its conversation surface: a persisted map
    key -> sid (pruned when the sid is in `gone`), a live provider per key that
    records whether it `session/load`ed the seeded sid, and `refuse` to model a
    provider that will not load a sid."""
    ...
    def resumable_sid(self, key):
        ...
        if sid in self.gone:
            del self.map[key]  # the real map self-prunes a sid whose session file is gone
            return None
        return sid

    def conversation_provider(self, key):
        return "acp" if key in self.map else ""

    def seed_conversation(self, key, sid, *, provider="", cwd=""):
        self.seeded.append((key, sid))
        self.map[key] = sid
    ...
    def start_turn(self, key):
        """What get_or_create does when the turn starts: load the seeded sid, or mint one."""
        sid = self.map.get(key)
        if sid and sid not in self.refuse:
            self.providers[key] = _provider_like(resumed=True, sid=sid)
        else:
            self.minted += 1
            fresh = f"sid-{self.minted}"
            self.map[key] = fresh
            self.providers[key] = _provider_like(resumed=False, sid=fresh)


class TestWakeConversation:
    """`members.<slug>.wake_context: resume` -- one durable conversation per member."""

    @pytest.fixture(autouse=True)
    def _turn_loads_session(self, wake_env, monkeypatch):
        mw, seen = wake_env
        orig = _Slot.enqueue_or_run_prompt

        def enqueue(self_slot, prompt, run, state):
            state.sessions.start_turn(f"dashboard:{self_slot.key}")
            return orig(self_slot, prompt, run, state)

        monkeypatch.setattr(_Slot, "enqueue_or_run_prompt", enqueue, raising=False)
        self.mw, self.seen = mw, seen
    ...
    @pytest.mark.asyncio
    async def test_consecutive_wakes_share_one_durable_conversation(self):
        """Wake 1 mints the conversation; wake 2 (a later batch, a fresh ephemeral
        slot) is seeded with the SAME sid and the provider `session/load`s it --
        the model context that answered wake 1's envelopes is the one wake 2
        runs in. The per-wake map entries are dropped; the sid lives in the
        member record and survives a restart."""
        inbox = InboxStore("radar")
        state = self._state()
        inbox.append(_env("radar", body="the deploy window is Tuesday 14:00"))
        r1 = await self.mw.run_member_wake(state, "radar")
        assert r1["ok"]
        conv = mi.read_conversation("radar")
        assert conv is not None and conv["sid"] == "sid-1"
        assert state.sessions.seeded == []  # nothing to resume yet: the first conversation
        assert state.sessions.forgotten == [f"dashboard:{r1['wake']}"]  # no per-wake map growth

        inbox.append(_env("radar", body="when is the deploy window?"))
        r2 = await self.mw.run_member_wake(state, "radar")
        assert r2["ok"] and r2["wake"] != r1["wake"]
        assert state.sessions.seeded == [(f"dashboard:{r2['wake']}", "sid-1")]
        assert state.sessions.providers[f"dashboard:{r2['wake']}"].client.resumed is True
        assert state.sessions.minted == 1  # one conversation, two turns
        assert mi.read_conversation("radar")["sid"] == "sid-1"
        assert not any(e.kind == "system" for e in OutboxStore("radar").rows())  # no reset
        assert "[CONTEXT RESET]" not in self.seen["prompt"]

    @pytest.mark.asyncio
    async def test_a_wake_defers_while_the_threads_own_turn_is_still_running(self):
        """The flag flipped while the thread had a turn in flight: that turn owns
        the provider session the wake would retire. The wake defers -- envelopes
        pending, no attempt counted, no session removed -- and the scheduler's
        retry picks them up once the turn has ended."""
        state = self._state()
        state.sessions.map["dashboard:member-radar"] = "thread-sid"
        state.sessions.providers["dashboard:member-radar"] = SimpleNamespace(resumed=True)
        state.thread.task = asyncio.get_running_loop().create_future()  # a live turn
        state.thread.running = True
        inbox = InboxStore("radar")
        inbox.append(_env("radar"))
        r = await self.mw.run_member_wake(state, "radar")
        assert r == {"ok": False, "reason": "thread_turn_running", "drained": 0}
        assert state.sessions.removed == [] and state.sessions.seeded == []
        assert len(inbox.pending()) == 1 and inbox.pending()[0].attempts == 0
        assert "prompt" not in self.seen
        state.thread.task.cancel()

    @pytest.mark.asyncio
    async def test_first_resume_adopts_the_live_threads_conversation(self):
        """Flipping the flag on loses nothing: the first wake continues the
        conversation the thread was having (its resumable sid), and the thread's
        idle provider is retired so one process holds the session."""
        state = self._state()
        state.sessions.map["dashboard:member-radar"] = "thread-sid"
        state.sessions.providers["dashboard:member-radar"] = SimpleNamespace(resumed=True)
        InboxStore("radar").append(_env("radar"))
        r = await self.mw.run_member_wake(state, "radar")
        assert r["ok"]
        assert state.sessions.removed == ["dashboard:member-radar"]
        assert state.sessions.seeded == [(f"dashboard:{r['wake']}", "thread-sid")]
        assert mi.read_conversation("radar")["sid"] == "thread-sid"

    @pytest.mark.asyncio
    async def test_session_gone_resets_before_the_turn_with_a_projection_row(self):
        """The persisted sid's native session is gone: the seed is pruned, the
        wake knows BEFORE the turn, records `wake_context_reset` in the
        projection, tells the model in the prompt, runs in a new conversation,
        and the member record now names that new conversation."""
        state = self._state()
        mi.write_conversation("radar", "old-sid", provider="acp")
        state.sessions.gone.add("old-sid")
        InboxStore("radar").append(_env("radar"))
        r = await self.mw.run_member_wake(state, "radar")
        assert r["ok"]
        resets = [e for e in OutboxStore("radar").rows() if e.kind == "system"]
        assert len(resets) == 1 and resets[0].refs[mi.WAKE_CONTEXT_RESET_REF] is True
        assert (
            resets[0].refs["reason"] == "session_gone" and "[wake_context_reset]" in resets[0].body
        )
        assert "[CONTEXT RESET]" in self.seen["prompt"] and "session_gone" in self.seen["prompt"]
        assert mi.read_conversation("radar")["sid"] == "sid-1"  # the new conversation
        # the envelopes were still acked: continuity is lost, never a message
        assert InboxStore("radar").pending() == []

    @pytest.mark.asyncio
    async def test_provider_refusing_the_load_resets_after_the_turn(self):
        """The sid is on record but the provider will not load it: the turn ran
        in a new conversation (detected from the provider's `resumed` flag), the
        reset row is written after the turn, the new sid is recorded."""
        state = self._state()
        mi.write_conversation("radar", "refused-sid", provider="acp")
        state.sessions.refuse.add("refused-sid")
        InboxStore("radar").append(_env("radar"))
        r = await self.mw.run_member_wake(state, "radar")
        assert r["ok"]
        (reset,) = [e for e in OutboxStore("radar").rows() if e.kind == "system"]
        assert reset.refs["reason"] == "provider_refused_resume"
        assert "[CONTEXT RESET]" not in self.seen["prompt"]  # only known after the turn
        assert mi.read_conversation("radar")["sid"] == "sid-1"

    @pytest.mark.asyncio
    async def test_a_provider_without_a_resumed_flag_is_judged_by_the_sid(self, monkeypatch):
        """A pod run wrote a `provider_refused_resume` row after EVERY wake: the
        flag was read off the outer provider object, which has none, so a
        successful resume looked like a refusal. A provider exposing no flag
        anywhere and leaving the seeded sid in place is a resume -- no row; the
        same provider re-keying the map to a new sid is a refusal -- one row."""
        state = self._state()
        mi.write_conversation("radar", "sid-kept", provider="acp")

        def start_turn_flagless(key, _orig=state.sessions.start_turn):
            sid = state.sessions.map.get(key)
            if sid in state.sessions.refuse:
                state.sessions.map[key] = "sid-minted"
                state.sessions.providers[key] = SimpleNamespace(sid="sid-minted")
            else:
                state.sessions.providers[key] = SimpleNamespace(sid=sid)

        monkeypatch.setattr(state.sessions, "start_turn", start_turn_flagless)
        InboxStore("radar").append(_env("radar"))
        assert (await self.mw.run_member_wake(state, "radar"))["ok"]
        assert not any(e.kind == "system" for e in OutboxStore("radar").rows())
        assert mi.read_conversation("radar")["sid"] == "sid-kept"

        state.sessions.refuse.add("sid-kept")
        InboxStore("radar").append(_env("radar"))
        assert (await self.mw.run_member_wake(state, "radar"))["ok"]
        (reset,) = [e for e in OutboxStore("radar").rows() if e.kind == "system"]
        assert reset.refs["reason"] == "provider_refused_resume"
        assert mi.read_conversation("radar")["sid"] == "sid-minted"

    def test_provider_resumed_reads_the_inner_client_flag(self):
        """Direct pins on the predicate: outer flag, inner flag, sid change, and
        a truthy non-bool that must not count."""
        f = self.mw._provider_resumed
        assert f(SimpleNamespace(client=SimpleNamespace(resumed=True)), "s", "s") is True
        assert f(SimpleNamespace(client=SimpleNamespace(resumed=False)), "s", "s") is False
        assert f(SimpleNamespace(resumed=False), "s", "s") is False
        assert f(SimpleNamespace(client=SimpleNamespace(resumed=True)), "s", "other") is False
        assert f(SimpleNamespace(client=SimpleNamespace(resumed=object())), "s", "s") is True
        assert f(SimpleNamespace(), "s", "s") is True

    @pytest.mark.asyncio
    async def test_fresh_mode_seeds_nothing_and_keeps_no_conversation(self, monkeypatch):
        monkeypatch.setattr(mi, "member_inbox_setting", lambda slug, key, default: "fresh")
        monkeypatch.setattr(mi, "wake_context_mode", lambda slug: mi.WAKE_CONTEXT_FRESH)
        state = self._state()
        mi.write_conversation("radar", "ignored-sid", provider="acp")
        InboxStore("radar").append(_env("radar"))
        r = await self.mw.run_member_wake(state, "radar")
        assert r["ok"]
        assert state.sessions.seeded == [] and state.sessions.minted == 1
        assert (
            mi.read_conversation("radar")["sid"] == "ignored-sid"
        )  # untouched, for a later `resume`
        assert state.sessions.forgotten == [f"dashboard:{r['wake']}"]
        assert not any(e.kind == "system" for e in OutboxStore("radar").rows())

    def test_wake_context_mode_defaults_to_resume_and_only_fresh_opts_out(self, monkeypatch):
        for raw, want in [
            ("fresh", mi.WAKE_CONTEXT_FRESH),
            ("resume", mi.WAKE_CONTEXT_RESUME),
            ("FRESH", mi.WAKE_CONTEXT_RESUME),
            (None, mi.WAKE_CONTEXT_RESUME),
            (7, mi.WAKE_CONTEXT_RESUME),
        ]:
            monkeypatch.setattr(mi, "member_inbox_setting", lambda slug, key, default, r=raw: r)
            assert mi.wake_context_mode("radar") == want

    @pytest.mark.asyncio
    async def test_flag_off_hands_the_conversation_back_to_the_thread(self, monkeypatch):
        """Rollback: the next message on the (unflagged) thread makes the thread's
        session continue the wakes' conversation, and the member record is cleared
        so a later re-flag adopts it back from the thread."""
        ...
        mi.write_conversation("nobody", "wake-sid", provider="acp")
        state = self._state()
        state.sessions.providers["dashboard:member-nobody"] = SimpleNamespace(resumed=True)
        slot = _Slot("member-nobody")
        slot.mode = "member"
        # A wake still running holds the conversation: the send is refused,
        # retryable, rather than run as an ordinary turn whose provider
        # conversation the later adoption would replace (its context lost).
        monkeypatch.setattr("kiro_crew.member_scheduler.member_wake_running", lambda slug: True)
        resp = await _member_inbox_intake(state, slot, "hi", None)
        assert resp is not None and resp.status == 409
        assert json.loads(resp.text)["code"] == "member_wake_running"
        assert state.sessions.removed == [] and mi.read_conversation("nobody") is not None
        assert slot.appended == []  # no turn, no row
        monkeypatch.setattr("kiro_crew.member_scheduler.member_wake_running", lambda slug: False)
        assert await _member_inbox_intake(state, slot, "hi", None) is None  # falls through
        assert state.sessions.removed == ["dashboard:member-nobody"]
        assert state.sessions.seeded == [("dashboard:member-nobody", "wake-sid")]
        assert mi.read_conversation("nobody") is None
...
class TestWakeRunner:
    ...
    async def test_reply_whose_mirror_failed_is_restored_by_the_next_wake(
        ...
        assert rows[row.id].refs[mi.VIEW_REF] == mi.VIEW_SHOWN
        assert OutboxStore("radar").view_pending() == []

    @pytest.mark.asyncio
    async def test_a_wake_whose_close_failed_is_evicted_not_left_registered(
        self, wake_env, monkeypatch
    ):
        """`close_slot` failing (its history save raised) restores the slot and
        skips the session teardown; a wake under a fresh key would then sit in the
        registry with its provider subprocess forever, one per interval. The
        runner evicts it: registry entry gone, session removed, transcript
        discarded, and the wake's outcome is unchanged."""
        import kiro_crew.dashboard.chat_handlers as ch
        ...
        async def failing_close(state, slot, key):
            # what the real close does on a failed save: put the slot back
            state._slots[key] = slot
            raise RuntimeError("history save failed")

        monkeypatch.setattr(ch, "close_slot", failing_close)
        inbox = InboxStore("radar")
        inbox.append(_env("radar"))
        state = _State("ok")
        state._slots = state.slots  # the registry the real state keeps
        state.sessions = _Sessions()
        state.push_slots_update = lambda: None
        r = await mw.run_member_wake(state, "radar")
        assert r["ok"] and inbox.pending() == []  # the wake itself succeeded
        assert r["wake"] not in state._slots  # evicted despite the failed close
        assert removed == [f"dashboard:{r['wake']}"]  # provider session torn down
        assert [k.split(":", 1)[-1] for k in state.conversation_log.deleted] == [r["wake"]]

    @pytest.mark.asyncio
    async def test_an_answered_envelope_at_the_attempt_ceiling_is_acked_not_dead_lettered(
        self, wake_env
    ):
        """Two wakes crashed on it, the third replied, stamped `completed` and died
        before its ack: the envelope sits at attempts == MAX_ATTEMPTS with a
        completed reply on record. The next wake must finish the ack -- not count
        one more attempt, dead-letter it and tell the owner it "crashed"."""
        mw, seen = wake_env
        inbox = InboxStore("radar")
        env = inbox.append(_env("radar", body="what did you triage?"))
        inbox.mark_attempt([env.id])
        inbox.mark_attempt([env.id])
        inbox.mark_attempt([env.id])  # attempts == MAX_ATTEMPTS (3): one more would dead-letter
        assert inbox.pending()[0].attempts == mi.MAX_ATTEMPTS
        OutboxStore("radar").append(
            kind=mi.OUTBOX_REPLY_KIND,
            body="Done: triaged it.",
            refs={"wake_key": "member-radar.wake-0", "in_reply_to": [env.id], "completed": True},
        )
        state = _State("ok")
        r = await mw.run_member_wake(state, "radar")
        assert r == {"ok": True, "drained": 1, "dead_lettered": 0, "already_answered": 1}
        assert inbox.pending() == [] and inbox.dead_letters() == []
        assert [e.id for e in inbox.acked()] == [env.id]
        assert inbox.acked()[0].attempts == mi.MAX_ATTEMPTS  # not incremented
        assert state.notified == []  # no "crashed" escalation for finished work
        assert "prompt" not in seen  # and no second turn

    @pytest.mark.asyncio
    async def test_unloaded_thread_restore_is_idempotent_on_the_record_id(
        self, wake_env, monkeypatch
    ):
        """Thread not loaded, the reply's transcript append landed but the process
        died before the `shown` stamp: the recovery pass re-appends by the row's
        record id (`append_if_absent(mid=<outbox id>)`), so the row is on disk
        exactly once."""
        from types import SimpleNamespace
        ...
        r = await mw.run_member_wake(state, "radar")
        assert r["ok"]
        (row,) = OutboxStore("radar").rows()
        assert state.conversation_log.mids == [row.id]
        # the stamp did not land (crash) -- put the record back to pending and recover
        OutboxStore("radar").mark(row.id, **{mi.VIEW_REF: mi.VIEW_PENDING})
        assert await mw.reconcile_thread_view(state, "radar", "member-radar") == 1
        assert state.conversation_log.appended == [
            ("dashboard:member-radar", "assistant", "Done: triaged it.")
        ]  # once
        assert OutboxStore("radar").rows()[0].refs[mi.VIEW_REF] == mi.VIEW_SHOWN
...
class TestInboxRoutes:
    ...
    @pytest.mark.asyncio
    async def test_reply_row_is_born_view_pending_before_any_mirror(self, monkeypatch):
        """Crash consistency: the record carries `view=pending` in ITS OWN durable
        write, not in a later stamp a crash could skip. With the mirror gone
        entirely (the process died there), the row on disk is still one the
        recovery scan finds."""
        from aiohttp.test_utils import TestClient, TestServer

        async def died(*a, **kw):
            raise RuntimeError("process gone")  # stands in for the crash; never caught here

        monkeypatch.setattr("kiro_crew.dashboard.member_wake._mirror_to_thread", died)
        inbox = InboxStore("radar")
        a = inbox.append(_env("radar"))
        ...
        async with TestClient(TestServer(_routes_app(state))) as client:
            ...
            assert resp.status == 500  # the handler died after the durable write
        (row,) = OutboxStore("radar").rows()
        assert row.refs[mi.VIEW_REF] == mi.VIEW_PENDING
        assert [e.id for e in OutboxStore("radar").view_pending()] == [row.id]
    ...
    async def test_outbox_send_strips_reserved_refs_and_stamps_the_batch(self):
        ...
        assert rows[0].refs == {
            ...
            "wake_key": wake.key,
            "in_reply_to": [a.id],
            mi.VIEW_REF: mi.VIEW_SHOWN,  # born pending, mirrored, stamped shown
        }
...
class TestSessionSendShim:
    ...
    @pytest.mark.asyncio
    async def test_redaction_expansion_past_the_envelope_cap_is_a_typed_refusal(self, monkeypatch):
        """A limit-sized body whose redaction GROWS it must be refused with
        `message_too_long`, not raise a bare `InboxError` out of `make_envelope`
        (a 500 to the caller)."""
        from kiro_crew.dashboard import session_control as sc
        ...
        monkeypatch.setattr(sc, "_resolve_slot", lambda st, target: slots.get(target))
        monkeypatch.setattr(sc, "caller_slot_key", lambda st, key: key)
        monkeypatch.setattr(sc, "authorize_target", lambda st, **kw: slots["member-fixer"])
        monkeypatch.setattr(sc, "_audit", lambda **kw: None)
        monkeypatch.setattr(sc, "sanitize_outbound", lambda text: text + "X" * 2)
        with pytest.raises(sc.SessionControlError) as exc:
            await sc._inbox_model_shim(
                state,
                caller_session_key="chat-1-1",
                target="member-fixer",
                body="a" * (mi.MAX_BODY_CHARS - 1),
            )
        assert exc.value.code == "message_too_long" and exc.value.status == 400
        assert InboxStore("fixer").pending() == []
...
class TestInboxIntake:
    ...
    async def test_failed_thread_save_is_stamped_and_restored_before_the_next_intake(
        ...
        order: list[str] = []
        monkeypatch.setattr(ms, "notify_member", lambda slug, kind: order.append("notify"))
        saves: list[str] = []
        outcome = {"ok": False}
        ...
        # the scheduler is told only after the save (and the stamp it failed into):
        # a wake woken earlier could rewrite the envelope under the stamp
        ...
        assert pend["first"].refs[mi.VIEW_REF] == mi.VIEW_SHOWN
        assert pend["second"].refs[mi.VIEW_REF] == mi.VIEW_SHOWN  # its own save succeeded
...
class TestConfigSections:
    ...
    @pytest.mark.parametrize(
        "section", ["yes", {"enabled": "yes"}, {"enabled": 1}], ids=["not-a-dict", "str", "int"]
    )
    def test_malformed_section_still_fails_closed(self, tmp_path, monkeypatch, caplog, section):
        """A malformed kill switch denies on EVERY host. Where `jsonschema` is
        installed the advisory validation prunes an invalid section before the
        dataclass is built and the loader would read the permissive default; the
        loader normalizes the value to `enabled: false` first and records the
        section as degraded, which is what the admission reads."""
        cfg = self._load(
            tmp_path, monkeypatch, caplog, {"members": ["radar"], "member_peer_dm": section}
        )
        from kiro_crew.config.loader import KiroCrewConfig

        assert "member_peer_dm" in cfg.degraded_sections
        assert cfg.member_peer_dm == {"enabled": False}
        assert any(
            "member_peer_dm" in r.getMessage() and "DENIED" in r.getMessage()
    ...
    def test_well_formed_switch_is_not_degraded(self, tmp_path, monkeypatch, caplog):
        cfg = self._load(tmp_path, monkeypatch, caplog, {"member_peer_dm": {"enabled": False}})
        from kiro_crew.config.loader import KiroCrewConfig

        assert "member_peer_dm" not in cfg.degraded_sections
        monkeypatch.setattr(KiroCrewConfig, "load", classmethod(lambda cls, *a, **k: cfg))
        assert _REAL_READERS["global"]() is False  # the operator's own off, not a degradation
