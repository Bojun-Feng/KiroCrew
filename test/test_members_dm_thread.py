...
class TestRosterFreeTextMasking:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("absent_owner", [None, ""])
    async def test_absent_memory_owner_is_not_masked(self, tmp_path, absent_owner):
        """No owner ships as ``""``, never as the sentinel.

        The loader passes a JSON ``null`` through as ``None`` and the mask
        treats every non-string as suspicious, so without normalizing first a
        V1 store with no owner would come back as the truthy sentinel and the
        frontend's ``memory_owner && memory_owner !== name`` check would flip
        the row from ``legacy`` to ``ownership_mismatch``.
        """
        from kiro_crew.dashboard.handlers.core import _SENSITIVE_MASK

        state = _make_state(tmp_path)
        record = SimpleNamespace(memory_version=1, owner_member=absent_owner)
        cfg = SimpleNamespace(
            agents={CREW: KiroCrewAgentConfig(kiro_agent=CREW, memory_store="own")},
            default_agent=CREW,
            memory_stores={"own": record},
        ...
        with patch(
            ...
            async with TestClient(TestServer(_make_members_app(state))) as client:
                ...
                data = await resp.json()
        row = data["members"][0]
        # Absence is preserved as the falsy empty string (the frontend's
        # ``legacy`` branch), not turned into the truthy mask.
        assert row["memory_owner"] == ""
        assert row["memory_owner"] != _SENSITIVE_MASK
