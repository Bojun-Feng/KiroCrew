...
* the KAS projection (``<id>.md`` fallback, body as prompt, JSON+md twin refused);
...
class TestParser:
    ...
    def test_iteration_covers_both_forms_and_only_those(self, tmp_path: Path) -> None:
        for name in ("b.md", "a.json", "README.txt", "c.json.bak", "a.md"):
            (tmp_path / name).write_text("x", encoding="utf-8")
        assert [p.name for p in iter_agent_spec_files(tmp_path)] == ["a.json", "a.md", "b.md"]
...
class TestKasProjection:
    ...
    def test_a_json_and_markdown_twin_is_refused(self, tmp_path: Path) -> None:
        """Neither declares the id; the same agent twice, and which one KAS
        would have run is undefined."""
        (tmp_path / "bot.json").write_text(json.dumps({"name": "other"}), encoding="utf-8")
        (tmp_path / "bot.md").write_text(_md("another"), encoding="utf-8")
        with pytest.raises(KasAgentTranslationError, match="2 spec files"):
...
class TestWriters:
    ...
    def test_agent_spec_path_refuses_a_same_stem_twin(self, agents_dir: Path) -> None:
        (agents_dir / "bot.json").write_text(json.dumps({"name": "x"}), encoding="utf-8")
        (agents_dir / "bot.md").write_text(_md("y"), encoding="utf-8")
        with pytest.raises(ValueError, match="same agent twice"):
            agent_mod.agent_spec_path("bot")
    ...
    def test_markdown_spec_for_agent_names_the_file(self, agents_dir: Path, tmp_path: Path) -> None:
        ...
        # The project checkout wins, as it does for kiro-cli's --agent.
        assert agent_mod.markdown_spec_for_agent("plain", proj) == d / "plain.md"
